...
const AdminCreateOrder = () => {
  ...
  const productOptions = useMemo(() => filteredProducts.flatMap((product) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      productService.getAllProducts({ limit: 100, search: query, forOrder: true })
        .then((response) => setProducts(response.data?.data || []))
        .catch(() => setError('Products could not be searched. Please try again.'))
  ...
  const handleCustomerSearchKeyDown = (event) => {
    ...
    if (event.key === 'ArrowDown') {
      ...
      setSelectedCustomerIndex((current) => Math.min(current + 1, filteredUsers.length - 1));
  ...
  const handleProductSearchKeyDown = (event) => {
    if (productSearchLoading || productOptions.length === 0) return;
    if (event.key === 'ArrowDown') {
      event.preventDefault();
      setSelectedProductIndex((current) => Math.min(current + 1, productOptions.length - 1));
    } else if (event.key === 'ArrowUp') {
      event.preventDefault();
      setSelectedProductIndex((current) => Math.max(current - 1, 0));
    } else if (event.key === 'Enter') {
      event.preventDefault();
      const selectedProduct = productOptions[selectedProductIndex];
      if (selectedProduct) addProduct(selectedProduct.product, selectedProduct.variant);
    }
  };
              ...
              <div>
                ...
                <div className="relative mt-2">
                  <input
                    ...
                    className="input-field search-clear-red w-full"
                    placeholder="Search by name, email, phone, or ID"
                    aria-label="Search customers"
                    aria-activedescendant={filteredUsers[selectedCustomerIndex] ? `customer-option-${selectedCustomerIndex}` : undefined}
                  />
                ...
                <div className="mt-2 max-h-56 overflow-y-auto rounded-lg border border-gray-200 bg-white" role="listbox" aria-label="Customer results">
                  {filteredUsers.length > 0 ? filteredUsers.map((user, index) => (
                    <button
                      ...
                      className={`block w-full border-b border-gray-100 px-3 py-2.5 text-left last:border-b-0 hover:bg-primary/5 ${index === selectedCustomerIndex ? 'bg-primary/5' : ''} ${form.user_id === user.id ? 'bg-primary/10' : ''}`}
                    >
            ...
            <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid gap-4 sm:grid-cols-[1fr_220px]">
                <div>
                  ...
                  <div className="relative mt-2">
                    <input id="product-search" type="search" value={productSearch} onChange={(event) => setProductSearch(event.target.value)} onKeyDown={handleProductSearchKeyDown} className="input-field search-clear-red w-full " placeholder="Search by product name, SKU, or ID" aria-label="Search products" aria-activedescendant={productOptions[selectedProductIndex] ? `product-option-${selectedProductIndex}` : undefined} />
                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" aria-hidden="true"></span>
                  </div>
                  <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border border-gray-200 bg-white" role="listbox" aria-label="Product results">
                    {productSearchLoading ? <p className="px-3 py-3 text-sm text-gray-500">Searching products...</p> : productOptions.length > 0 ? productOptions.map(({ product, variant }, index) => {
                        const variantKey = variant?.sku || variant?.name || variant?.label || '';
                        ...
                        return (
                          <button type="button" id={`product-option-${index}`} role="option" aria-selected={index === selectedProductIndex} key={optionKey} onClick={() => addProduct(product, variant)} className={`flex w-full items-center justify-between border-b border-gray-100 px-3 py-2.5 text-left last:border-b-0 hover:bg-primary/5 ${index === selectedProductIndex ? 'bg-primary/5' : ''}`}>
                ...
                <label className="block text-sm font-semibold text-gray-700">Payment method
                  <span className="relative mt-2 block">
                    <select value={form.payment_method} onChange={(event) => updateForm('payment_method', event.target.value)} className="input-field select-field w-full">
